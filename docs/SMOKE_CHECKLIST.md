...
- [ ] "S" icon appears in the menu bar (no Dock icon).
- [ ] Click "Enable NoSleep" → S becomes struck through; `pmset -g assertions`
      shows `PreventUserIdleSystemSleep` held by NoSleep.
- [ ] Click "Disable NoSleep" → strike clears; assertion clears from
      `pmset -g assertions`.
- [ ] Press ⌃⌘S → toggles state; menu label updates.
